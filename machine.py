...
class MACHINE():
    ...
    def find_best_selection(self):
        ...
        if (lines := self.check_get2point()) != None: # lines = None or [line, line ...]
            # if self.minmax_mode:
            #     line = self.minmax(self.search_depth, self.search_depth, float("-inf"), float("inf"), float('-inf'), float('inf'), self.drawn_lines_copy, [0,0], True, lines)
            # else:
            #     line = lines[0]
            line = lines[0]
            print("(get 2 point)choice : "+str(line))
            # return line
        # 1점 얻을 수 있는 액션 있는지 확인, 있으면 낚시 상황 있는지 확인
            ...
            if not is_good:
                print("Not good 1 point actions")
                lines = list(self.get_available_lines(self.drawn_lines_copy))
                random.shuffle(lines)
                _, line = self.minmax(self.search_depth, self.search_depth, float("-inf"), float("inf"), float('-inf'), float('inf'), self.drawn_lines_copy, [0,0], True, lines[:self.candidate_num])
            
            else:
                print("(get 1 point)choice : "+str(line))
            # return line
        # 상대방이 낚시에 당하면 낚시 시도
            ...
            if self.minmax_mode:
                is_good, line = self.minmax(self.search_depth, self.search_depth, float("-inf"), float("inf"), float('-inf'), float('inf'), self.drawn_lines_copy, [0,0], True, lines)
                if not is_good:
                    print("Not good fooling actions")
                    lines = list(self.get_available_lines(self.drawn_lines_copy))
                    random.shuffle(lines)
                    _, line = self.minmax(self.search_depth, self.search_depth, float("-inf"), float("inf"), float('-inf'), float('inf'), self.drawn_lines_copy, [0,0], True, lines[:self.candidate_num])
            else:
                line = lines[0]
            print("(fooling triangle)choice : "+str(line))
            # return line
            ...
            if line != None:
                ...
                if temp == None: # 이번이 마지막 no score action 이었으면 minmax 돌리기
                    ...
                    print("minmax mode")
                print("END")
                print("(no score action)choice : "+str(line))
            ...
            if self.minmax_mode:
                is_good, line = self.minmax(self.search_depth, self.search_depth, float("-inf"), float("inf"), float('-inf'), float('inf'), self.drawn_lines_copy, [0,0], True, lines)
                if not is_good:
                    print("Not good pingpong actions")
                    lines = list(self.get_available_lines(self.drawn_lines_copy))
                    random.shuffle(lines)
                    _, line = self.minmax(self.search_depth, self.search_depth, float("-inf"), float("inf"), float('-inf'), float('inf'), self.drawn_lines_copy, [0,0], True, lines[:self.candidate_num])
            else:
                line = lines[0]
            print("(ping pong)choice : "+str(line))
    ...
    def find_unconnected_lines(self):
        ...
        for unconnected_line_set in unconnected_lines:
            ...
            for line in [[(x2, y2), (x3, y3)], [(x4, y4), (x1, y1)], [(x1, y1), (x3, y3)], [(x2, y2), (x4, y4)]]:
                if self.organize_points(line) != self.organize_points(line1) and self.organize_points(line) != self.organize_points(line2) and self.check_availability(line):
...
####################################################### 한승현 #######################################################
    ...
    def minmax(self, init_depth, depth, parent_alpha, parent_beta, alpha, beta, cur_map, score, maximizing_player, search_line_list):
        ...
        if depth == 0 or len(self.get_available_lines(cur_map)) == 0:  # 윗줄처럼 하려 했지만 종료 상태 아님이 확실하기 때문에 depth만 확인해줘도 될 것 같다
            ...
            # eval += 0.5 * self.check_get1point_h()
            # eval += 1.0 * self.check_get2point_h()

            # self.check_get1point()
            # self.check_get2point()
            # self.find_unconnected_lines()
            # print(score)
            # print("leaf node")
            # print("eval: ", eval)
        ...
        if maximizing_player:  # max layer인지 체크
            ...
            for line in lines:  # 가능한 노드들 탐색 함수. 이미 결과값 있다면 해당 points 리스트로 변경 가능
                ...
                changed_score = [score[0], score[1] + self.check_triangleCount_map(line, next_map) * (0.95 ** (init_depth - depth))]
            ...
            if init_depth == depth: # root일 때
                if alpha < 0:
                    return False, max_line
                else:
                    return True, max_line
            else:
            ...
            for line in lines:  # 가능한 노드들 탐색 함수. 이미 결과값 있다면 해당 points 리스트로 변경 가능
                ...
                cur_map_copy.append(line)
                next_map = cur_map_copy
                changed_score = [score[0] + self.check_triangleCount_map(line, next_map)  * (1.02 ** (init_depth - depth)), score[1]]
